...
smooth = -10
factor = 1/float(len(glob(f"{src}/*.cif"))) # 1/N_atoms, normalize
...
def main(name):
    print(name)
    mof = CIF2PandasAdapter().apply(name)
    # mof = mof[mof.visible_keys[0]]
    metadata = mof.metadata.set_index(0)

    elements = list(mof.loops[0]["_atom_site_type_symbol"])
    ...
    la = float(metadata.loc["_cell_length_a"])
    lb = float(metadata.loc["_cell_length_b"])
    lc = float(metadata.loc["_cell_length_c"])
    aa = np.deg2rad(float(metadata.loc["_cell_angle_alpha"]))
    ab = np.deg2rad(float(metadata.loc["_cell_angle_beta"]))
    ag = np.deg2rad(float(metadata.loc["_cell_angle_gamma"]))
    # If volume is missing from .cif, calculate it.
    try:
        cv = float(metadata.loc["_cell_volume"])
    except KeyError:
    ...
    frac = np.array([
        list(mof.loops[0]["_atom_site_fract_x"]),
        list(mof.loops[0]["_atom_site_fract_y"]),
        list(mof.loops[0]["_atom_site_fract_z"]),
    ], dtype=float).T
